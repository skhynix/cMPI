...
def format_large_number(num):
    """将大于4096的数字转换为"系数 * 10^n"形式"""
    if num > 64:
        # 计算对应的幂
        power = int(np.log10(num) // 1)  # 向下取整获取幂
        # 计算系数
        coefficient = num / (10 ** power)
        # 格式化输出，系数保留1位小数
        # return f"{coefficient:.1f}e{power}"
        return f"${coefficient:.1f} \\times 10^{{{power}}}$"
    return f"{num:.1f}"
...
def plot_bandwidth_subplots(eval_name, versions, process_nums, output_file=None, title=None, results_base_dir=None):
    ...
    for i, ver in enumerate(versions):
        ...
        yticks = [16, 64, 256, 1024, 4096, 16384, 65536, 262144, 1048576, 4194304]
        ytickslabels = []
        for t in yticks:
            ytickslabels.append(format_large_number(t))
